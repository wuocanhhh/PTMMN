...
class MessageDatabaseHandler(context: Context) :
    ...
    fun addMessage(message: ContentValues): Boolean {
        val db = this.writableDatabase
        val result = db.insert(TABLE_MESSAGES, null, message)
        db.close()
        return result != -1L
    }
    ...
    fun getMessagesFromConversation(conversationId: Int): Cursor {
        val db = this.readableDatabase
        val selectQuery = "SELECT * FROM $TABLE_MESSAGES WHERE $KEY_CONVERSATION_ID = ? ORDER BY $KEY_TIMESTAMP DESC"
        return db.rawQuery(selectQuery, arrayOf(conversationId.toString()))
    }
